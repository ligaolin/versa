...
class AdminCate extends Base
{
    ...
    function Edit(){
        ...
        if($data['id']){ // 更新
            ...
            if(self::Db()->where('path',$data['path'])->where('id','<>',$data['id'])->where('pid','<>',0)->count()) throw new \Exception('栏目跳转路径已存在');
            ...
            if(self::Db()->where('path',$data['path'])->where('pid','<>',0)->count()) throw new \Exception('栏目跳转路径已存在');
