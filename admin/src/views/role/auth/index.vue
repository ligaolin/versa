<template>
    <list :getData="getData" :editPage="edit" ref="listEleme" >
        <template #search>
            ...
            <label>权限名称：<el-input clearable v-model="name"/></label>
            <label>路由：<el-input clearable v-model="route"/></label>
            <label>状态：
...
const getData = ()=>{
    ...
    if(route.value) param.route = route.value
    if(state.value) param.state = state.value
    return ['UserAuthList',param,{type:'管理员'}]
}

const listEleme = ref(null)
const del = (ids)=>{
    listEleme.value.Del('UserAuthAdminDel',ids)
}
...
const change = (changeField,changeVal,whereVal)=>{
    listEleme.value.Change('UserAuthAdminChange',changeField,changeVal,whereVal)
}
</script>